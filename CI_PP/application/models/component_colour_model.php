...
class Component_colour_model extends MY_Model {
     ...
     */    
    public $_table = 'pp_component_colour';
    ...
    public function get_component_colour_by_id( $componentColourId ) {
        $result = $this->component_colour_model->get($componentColourId);

        if (!$result) {
            return NULL;
        } else {
            $loaded_component_colour = new Component_colour_model();
            $loaded_component_colour->instantiate(
                    $result->cmpnt_clr_value,
                    $result->cmpnt_clr_component_id
                    
            );
            $loaded_component_colour->setId($result->cmpnt_clr_id);

            return $loaded_component_colour;
        }
    }
